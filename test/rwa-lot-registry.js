...
describe("RwaLotRegistry", () => {
  ...
  it("rejects minting for invalid lot or invalid amount", async () => {
    ...
    await expect(
      registry.connect(originator).mintLotTokens(0, holder.address, 1)
    ...
    await expect(
      registry.connect(originator).mintLotTokens(2, holder.address, 1)
    ).to.be.revertedWithCustomError(registry, "LotNotFound");
  ...
  it("rejects redemption for invalid lot or invalid amount", async () => {
    ...
    await expect(
      registry.connect(holder).burnRedemption(0, holder.address, 1, "REDEEM")
    ).to.be.revertedWithCustomError(registry, "LotNotFound");
    ...
    await expect(
      registry.connect(holder).burnRedemption(2, holder.address, 1, "REDEEM")
    ).to.be.revertedWithCustomError(registry, "LotNotFound");

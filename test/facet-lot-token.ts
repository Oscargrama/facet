...
describe("FacetLotToken", () => {
  ...
  it("allows mint and burn only for minter role", async () => {
    const { admin, minter, holder, token } = await deployFixture();
  ...
  it("pauses transfers and minting", async () => {
    ...
    await expect(token.connect(holder).transfer(other.address, 1)).to.be.revertedWith("Pausable: paused");
    await expect(token.connect(minter).mint(holder.address, 1)).to.be.revertedWith("Pausable: paused");
